...
1. Copy the whole `service-health-endpoint/` directory into the target repo's
   `.agents/skills/` — `references/` travels with it.
2. Mirror it to other agent toolchains in use (symlink keeps one source of truth):
   ```bash
   mkdir -p .claude/skills .opencode/skills
   ln -s ../../.agents/skills/service-health-endpoint .claude/skills/service-health-endpoint
   ln -s ../../.agents/skills/service-health-endpoint .opencode/skills/service-health-endpoint
   ```
3. Ensure the repo has a root `AGENTS.md` naming this skill.
4. Nothing here is stack-specific except `references/`. If the target repo uses a
   stack with no reference file, follow `references/porting-guide.md` and add one.
