...
name: python-testing
description: >
  How OneSpace Python services are tested: pytest layout that mirrors `src/`,
  shared fixtures, offline-by-default mocking (httpx.MockTransport / seam
  fakes), and the 80% coverage floor. Use when writing or changing a test, when
  adding or editing any module under `src/` (the test ships in the same change),
  when a test fails or coverage drops, or when asked to "add tests", "check
  coverage", "why is the suite failing", "structure the tests". Self-contained —
  works unchanged in any repo it is copied into.
---
...
1. Copy the whole `python-testing/` directory into the target repo's
   `.agents/skills/`.
2. Mirror it to other agent toolchains in use (symlink keeps one source of truth):
   ```bash
   mkdir -p .claude/skills .opencode/skills
   ln -s ../../.agents/skills/python-testing .claude/skills/python-testing
   ln -s ../../.agents/skills/python-testing .opencode/skills/python-testing
   ```
3. Ensure the repo has a root `AGENTS.md` naming this skill as mandatory.
4. Add the `[dependency-groups] dev`, `[tool.pytest.ini_options]` and
   `[tool.coverage.*]` blocks from the *Commands* section to the target's
   `pyproject.toml`, then `uv sync --group dev`.
5. Copy `busl-licence-compliance/` alongside it — step 6 of the working procedure
   depends on it.
