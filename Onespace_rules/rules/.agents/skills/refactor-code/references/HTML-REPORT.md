...
The architectural review is rendered as a single HTML file at `agent-tracking/reports/<slug>.html` (or as Markdown with Mermaid code blocks if the user chose MD). Tailwind and Mermaid both come from CDNs — the one report type allowed external scripts, because diagrams are the point; it must contain no secrets or security findings. Mermaid handles graph-shaped diagrams reliably; hand-built divs and inline SVG handle the more editorial visuals (mass diagrams, cross-sections). Mix the two: don't lean on Mermaid for everything, it'll start to look generic.
