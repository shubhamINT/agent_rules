---
name: hirebot-fe-structure-app
description: >
  Detect, choose, record, and apply the folder structure of a HireBot React
  ...
  the first check of every hirebot-fe-workflow `feature` and `refactor` task.
...
Collect evidence as paths ("`src/components/JobsPage.tsx:40` calls `fetch`").
...
- **Mixed or unstructured** → recommend **feature-based** (the default in
  ...
  1. **Feature-based** — *default.* Each business area (`jobs`,
     `candidates`, `interviews`) owns its components, hooks, and API calls;
...
`hirebot-fe-workflow/references/VISUAL-REPORT.md`) with a Current → Target tree
...
(`hirebot-fe-workflow/references/README-STANDARD.md`). Log the decision in
`progress.md`.
...
- **Moves** are planned, approved steps: `git mv` to keep history, update
  ...
  must not change (`hirebot-fe-verify-ui` before and after).
