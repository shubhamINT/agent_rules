---
name: hirebot-fe-verify-ui
description: >
  Prove a HireBot React UI works and looks right by running it in headless
  ...
  Required final step of every hirebot-fe-workflow `feature`, `ui`,
...
another task): load `hirebot-fe-workflow` and run its phases 1–3 before
...
node <this-skill>/scripts/shoot.mjs \
  --url http://localhost:5173/jobs --name jobs \
  --url http://localhost:5173/jobs/42 --name job-detail \
...
`.../<slug>/refs` (see `hirebot-fe-design-ui/references/inspiration.md`).
...
- **Responsive** — each width has a layout chosen for it
  (`hirebot-fe-design-ui/references/responsive.md`); navigation works at
...
- **Design** — nothing from `hirebot-fe-design-ui/references/anti-patterns.md`.
...
- Link the final screenshots in the report's Screenshots section
  (`hirebot-fe-workflow/references/VISUAL-REPORT.md`).
- Put the check results in Verification: per page and width, console errors,
  failed requests, overflow, axe.
- Pass the images to the critique step of `hirebot-fe-design-ui`: what still
