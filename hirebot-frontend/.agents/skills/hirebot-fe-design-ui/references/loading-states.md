...
| Slow | still loading after ~2–3s | the skeleton **plus** a short message ("Still loading candidates…") |
| Very slow / long job | > ~8–10s, or a known long AI job | progress steps, partial results, and a way to leave and come back |
| Empty | success, zero items | explanation + the next action ("No candidates yet. Share the job link or import a CSV.") |
...
HireBot runs AI work (screening, parsing, generating questions) that can take
seconds to minutes. Waiting is fine; waiting blind is not.

- **Say what is happening** in steps: "Reading resume → Matching skills →
  Scoring" with the current step highlighted, driven by the job status from
  the backend (not a fake timer).
- **Show partial results** as they arrive: stream text token by token; add
  candidates to the list as each is scored.
