...
Best-in-class product UI to study for a recruiting SaaS (open the live
...
(empty states, onboarding), Ashby and Greenhouse (recruiting domain patterns).

## How to research

1. **Search for the screen, not the style**: "applicant tracking kanban",
   "candidate profile page", "SaaS pricing page", "analytics dashboard dark".
2. **Pick 3–5** that fit the design read and dials. Mix one from the recruiting
   domain with ones from outside it.
3. **Screenshot them** with `hirebot-fe-verify-ui` so the evidence stays in
...
4. **Write one line per reference** in `research/<slug>.md`:
   ...
   |-----------|--------------|------------------|
   | Linear issues list | 32px rows, status as a small coloured dot, hover reveals actions | candidate list |
   | Attio record page | two-column record, activity timeline on the right | candidate profile |
...
look like HireBot, not like Linear with another name.
