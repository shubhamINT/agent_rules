...
2. **`AGENTS.md` and `CLAUDE.md` are written by the agent.** On every
   non-trivial task, `hirebot-be-workflow` makes sure the repo root has:

   - `AGENTS.md` with the pack block between `<!-- hirebot-be:start -->` and
     `<!-- hirebot-be:end -->` (always-on rules, task routing, skill index). Only
   ...
   The block's source is `hirebot-be-workflow/templates/AGENTS.md`; the skills carry
   it, so plugin and `npx skills` installs get it too. To add it up front:

   ```bash
   curl -o AGENTS.md https://raw.githubusercontent.com/shubhamINT/agent_rules/master/hirebot-backend/.agents/skills/hirebot-be-workflow/templates/AGENTS.md
