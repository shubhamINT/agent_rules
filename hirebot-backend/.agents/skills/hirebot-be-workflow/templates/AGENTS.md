...
<!-- Managed by the hirebot-backend skill pack (hirebot-be-workflow, "Register the
pack"). Agents refresh the text between the hirebot-be markers; edit the pack, not
...
This repo uses the `hirebot-be-*` skills. Before any code change, load
`hirebot-be-coding-standards`. For any non-trivial task, load `hirebot-be-workflow`
first; it routes to the other skills. Find them by name (Claude Code plugin
skills appear as `hirebot-backend:hirebot-be-<skill>`).
...
```bash
npx skills add shubhamINT/agent_rules      # pick every hirebot-be-* skill
```
...
6. **Spend tokens on evidence, not words.** Search before reading, read a
   ...
   tokens (`hirebot-be-workflow/references/TOKEN-ECONOMY.md`).
...
<!-- Repo-owned. Filled in per repository by the `hirebot-be-structure-service` skill, after the user
