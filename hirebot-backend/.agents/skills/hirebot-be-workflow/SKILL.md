...
Paths like `hirebot-be-<skill>/references/<file>.md` name a sibling skill: resolve
...
|------|--------------------|
| `feature` | `hirebot-be-build-feature` |
| `refactor` | `hirebot-be-refactor-code` |
| `audit` | `hirebot-be-audit-code` |
| `security` | `hirebot-be-audit-security` |
| `test` | `hirebot-be-write-tests` |
| `debug` | `hirebot-be-debug` |
| `structure` | `hirebot-be-structure-service` |
| `health` | `hirebot-be-add-health-endpoint` |
...
- **project structure** — read the `## Project structure` section of the repo's
  ...
  refactor)? Run `hirebot-be-structure-service` now and settle the structure with the user
...
- baseline measurements — run what the repo already has: tests, coverage,
  linters, type checker, and for security the scanners in `hirebot-be-audit-security`.
...
- design notes for features: the system design pass
  (`hirebot-be-build-feature/references/system-design.md`)
...
Before the first edit, load `hirebot-be-coding-standards`.
