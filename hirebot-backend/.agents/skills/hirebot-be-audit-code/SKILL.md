...
description: >
  ...
  — after scope is agreed via hirebot-be-workflow. Produces a traceable findings report
  that names each smell and the refactoring that fixes it; fixes nothing unless
  remediation is in scope. For vulnerabilities use hirebot-be-audit-security (they can run
  together).
---
...
Prerequisite: `hirebot-be-workflow` phases 1–3 (request recorded, scope agreed). This skill
...
| Need | Read |
|------|------|
| The quality bar, finding severity, review dimensions | `hirebot-be-coding-standards/SKILL.md` |
| Smell names → refactoring names | `hirebot-be-coding-standards/references/smells-and-refactorings.md` |
| Language idioms | `hirebot-be-coding-standards/references/python-style.md`, `typescript-style.md` |
| The repo's structure rules | `AGENTS.md` → `## Project structure` (or `hirebot-be-structure-service`) |
...
Run what the repo has, or the tools in `hirebot-be-coding-standards/references/tooling.md`
...
- dependency audit (`pip-audit` / `npm audit`) — flag, and hand security depth to `hirebot-be-audit-security`
...
(`hirebot-be-workflow/references/SUBAGENTS.md`). You own the report: open
...
6. **AI-generated code** — the eight checks in `references/ai-generated-code.md`.
   ...
   confidently.
7. **Security smell spotted?** Record it briefly and recommend `hirebot-be-audit-security`
...
`hirebot-be-audit-security` with security severity.

## Phase 7 — Report

`hirebot-be-workflow` report template in the agreed format (HTML default). Visuals per
`hirebot-be-workflow/references/VISUAL-REPORT.md`: severity tiles and severity bar, one
...
- recommended follow-up tasks, each scoped (e.g. "hirebot-be-refactor-code: split
