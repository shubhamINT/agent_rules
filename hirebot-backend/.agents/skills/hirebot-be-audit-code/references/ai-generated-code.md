...
Used by `hirebot-be-audit-code` (review of any code), `hirebot-be-refactor-code` (smell inventory),
and as the self-review in `hirebot-be-coding-standards` before handing back your own code.
General quality is in `hirebot-be-coding-standards`; vulnerabilities in `hirebot-be-audit-security`.
...
Verify: apply the deletion test (`hirebot-be-coding-standards/references/module-design.md`) to each abstraction. One
