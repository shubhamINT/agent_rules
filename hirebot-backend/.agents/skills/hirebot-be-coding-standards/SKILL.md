---
name: hirebot-be-coding-standards
description: >
  ALWAYS-ON HireBot coding standard for Python and Node.js/TypeScript
  ...
  lint/format config; it is the yardstick for hirebot-be-build-feature, hirebot-be-refactor-code and
  hirebot-be-audit-code.
---
...
(HireBot deliberately differs, with the reason), `n/a` — so no Google rule is
...
| AI-generated code failure patterns (full detail) | `hirebot-be-audit-code/references/ai-generated-code.md` |
...
3. **Do not silently fix it** if it is outside the agreed scope. Offer it as a
   follow-up task (`hirebot-be-audit-code`, `hirebot-be-audit-security`, `hirebot-be-refactor-code`), because an
...
`hirebot-be-coding-standards/references/module-design.md`) decides whether an abstraction earns its keep.
...
Tests are part of the code quality bar, not an extra. See `hirebot-be-write-tests`. Headlines: test behaviour through the interface; one behaviour
...
9. **Security** — trust boundaries touched? then run the `hirebot-be-audit-security` quick checklist.

Cite smells and fixes **by name** (see `references/smells-and-refactorings.md`);
the full audit finding format is in `hirebot-be-audit-code`. Severity for quality findings:
...
verification steps in `hirebot-be-audit-code/references/ai-generated-code.md`:
