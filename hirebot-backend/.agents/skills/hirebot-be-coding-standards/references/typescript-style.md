# TypeScript / Node.js Style — HireBot Essentials
...
- **Security (§6)** — see `hirebot-be-audit-security/references/node-checks.md`.
...
hand in review; `override` = HireBot deliberately differs (reason given);
...
| 3.5.1 | Import type | override | Google permits `import type`; HireBot requires it for type-only imports so type and value imports stay explicit. |
...
| 4.4.5 | Class members | override | Google bans `#private`; HireBot allows it where the repo already uses it (native in Node 24), never mixed with `private`. |
...
| 4.9.5 | Equality checks | override | Google always allows `== null`; HireBot allows it only where the repo already uses it. Otherwise `===` / `!==` only. |
...
| 4.11.3 | Const enums | override | Google bans `const enum` but allows plain `enum`; HireBot avoids both, using string-literal unions or `as const` objects. |
...
| 6.1.1 | Return types | override | Google leaves return types optional; HireBot (allowed local policy) annotates parameters and return types on exported functions. |
| 6.2 | Undefined and null | override | Google has no preference; HireBot uses `undefined` for absence in new code, never mixing both for one meaning. |
...
| 7.2 | Conformance | n/a | tsetse and tsec are Google-internal; HireBot uses ESLint, eslint-plugin-security and Semgrep (`tooling.md`). |
