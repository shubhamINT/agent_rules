---
name: hirebot-be-debug
description: >
  Evidence-first debugging of HireBot Python (FastAPI) and Node.js
  ...
  only says "fix this". Runs inside hirebot-be-workflow as task type `debug`.
---
...
`hirebot-be-workflow` phases 1–2 now (task type `debug`, request recorded).
...
1. a failing test at the mirror path (`hirebot-be-write-tests`) — best, it
...
parallel (`hirebot-be-workflow/references/SUBAGENTS.md`); brief each with the
...
(`hirebot-be-workflow/references/SAFE-DELIVERY.md`). If the proper fix is
...
`hirebot-be-workflow` report template and
`hirebot-be-workflow/references/VISUAL-REPORT.md`. Required: symptom, root
