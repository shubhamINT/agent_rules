...
`hirebot-be-refactor-code` task with its own scope and plan.

Vocabulary is mandatory: read `hirebot-be-coding-standards/references/module-design.md` first and use its terms
...
- Layer rules from `hirebot-be-structure-service` are broken.
...
remote-but-owned, true external) per `hirebot-be-coding-standards/references/DEEPENING.md`; the
...
`hirebot-be-workflow/references/VISUAL-REPORT.md`.
...
- Start a new `hirebot-be-workflow` task (`hirebot-be-refactor-code` type) for that candidate.
- To explore interface alternatives, use `hirebot-be-coding-standards/references/DESIGN-IT-TWICE.md`
