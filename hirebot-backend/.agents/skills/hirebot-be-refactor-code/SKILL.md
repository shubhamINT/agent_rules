...
description: >
  ...
  and plan are agreed through hirebot-be-workflow. Detects the project structure first and
...
Prerequisite: `hirebot-be-workflow` phases 1–3 are done (scope agreed, tracking
created). This skill supplies the content of phases 4–7.

Yardsticks: `hirebot-be-coding-standards` (what good looks like),
`hirebot-be-coding-standards/references/smells-and-refactorings.md`
(names), `hirebot-be-coding-standards/references/module-design.md` (module and seam vocabulary), and the repo's recorded
project structure (`AGENTS.md` → `## Project structure`, set by `hirebot-be-structure-service`).
...
0. **Structure**: read the repo's recorded structure (`AGENTS.md` →
   `## Project structure`). None recorded? Run `hirebot-be-structure-service` first: detect
...
5. **Smell inventory**: each smell with Fowler name, `file:line`, and why it hurts here. Apply `hirebot-be-audit-code/references/ai-generated-code.md` too if the code was AI-generated.
...
(`hirebot-be-workflow/references/SUBAGENTS.md`).
...
- Delete dead code instead of commenting it out. Delete tests that only tested
  ...
  layer — see `hirebot-be-coding-standards/references/DEEPENING.md`).
...
Use `hirebot-be-workflow` report template and `hirebot-be-workflow/references/VISUAL-REPORT.md`.
