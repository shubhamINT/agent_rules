---
name: hirebot-be-build-feature
description: >
  How to add new behaviour to a HireBot Python (FastAPI) or Node.js
  ...
  boundary, docs in the same change. Use whenever the user asks to
  ...
  job that…" — after scope and plan are agreed through hirebot-be-workflow.
---
...
2. **Read the neighbourhood** — the repo's recorded structure (`AGENTS.md` →
   `## Project structure`; if missing, run `hirebot-be-structure-service` first), the closest existing
...
4. **External facts** — for any library or upstream API involved, read the
   ...
   `research/<slug>.md`. Never code against a remembered API (`hirebot-be-audit-code/references/ai-generated-code.md` §1).
...
- **Module interface**: smallest interface that covers the acceptance criteria
  (`hirebot-be-coding-standards/references/module-design.md`: deep modules, no speculative seams — one adapter is not a
...
Steps in `plan.md` follow `hirebot-be-workflow/references/SAFE-DELIVERY.md`:
...
1. **Tests first where the behaviour is clear**: write the failing test for the
   ...
   (`hirebot-be-workflow/references/SUBAGENTS.md`) — it writes the tests from
...
4. **No new dependency** unless the plan approved it — then verify it exists,
   is maintained, and licence-compatible.
5. **Docs in the same change**: README endpoint table, env table, structure
   tree, data stores; docstrings / JSDoc on public functions.
6. Log each step in `progress.md`.
...
- The AI-generated-code self-review in `hirebot-be-coding-standards` (all eight checks).
- `hirebot-be-audit-security` checklist items for every trust boundary the feature touched.
- `hirebot-be-coding-standards` checklist.
- Gates: formatter, linter, type checker, full test suite, coverage ≥ 80%.
