...
| **Hexagonal (ports & adapters)** | Domain core in the centre; HTTP, DB, queues are adapters plugged into ports the core defines | Heavy domain logic that must be tested without infrastructure; several real adapters per port | Ports with one adapter (a hypothetical seam — see `onespace-be-coding-standards` module design) |
