...
| `references/onespace-python-layout.md` | Variant used by existing OneSpace Python services |
| `references/onespace-node-layout.md` | Variant used by existing OneSpace Node services |
...
| OneSpace variant | `src/api/routes`, `src/services/<upstream>/` clients, `src/core/config.py` (see variant references) |
...
- **Unstructured or mixed** (any size) → recommend **layered, grouped by
  ...
  4. **OneSpace variant** — only recommend it when the repo already follows it
