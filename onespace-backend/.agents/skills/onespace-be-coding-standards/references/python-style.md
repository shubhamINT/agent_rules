# Python Style — OneSpace Essentials
...
| Topic | Google says | OneSpace rule |
|-------|-------------|---------------|
...
Status: `tool` = enforced automatically by the named tool; `rule` = follow it by hand (one-line summary); `override` = OneSpace deliberately differs (reason given); `n/a` = not applicable.
...
| 3.8.2 | Modules | tool | Every file carries the BUSL-1.1 licence header (checked by `add_license_headers.py --check`) and a module docstring (Ruff `D100`). |
