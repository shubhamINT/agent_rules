# JavaScript Style — OneSpace Essentials

OneSpace services are TypeScript and follow `typescript-style.md`; for `.ts`
...
`rule` means follow it by hand, as written; `override` means OneSpace
...
| 9.1.1 | Type annotations and other Closure Compiler annotations | n/a | Closure Compiler annotation reference; no Closure Compiler in OneSpace. |
