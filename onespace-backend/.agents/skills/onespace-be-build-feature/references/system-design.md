...
- Migrations: reversible; for existing tables follow expand → migrate →
  contract (`onespace-be-workflow/references/SAFE-DELIVERY.md`).
...
Covered by `onespace-be-build-feature` Phase 6 step 3 and `onespace-be-audit-security`.
...
`onespace-be-coding-standards/references/DESIGN-IT-TWICE.md` before committing
to one.
