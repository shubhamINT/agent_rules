---
name: onespace-fe-debug
description: >
  Evidence-first debugging of the OneSpace React frontend (Vite SPA or Next.js):
  ...
  user only says "fix this". Runs inside onespace-fe-workflow as task type
  `fix`.
---
...
`onespace-fe-workflow` phases 1–2 now (task type `fix`, request recorded).
...
2. **A browser repro with `scripts/capture.mjs`** — for bugs that need a real
   ...
   ```bash
   node <this-skill>/scripts/capture.mjs --url http://localhost:5173/projects \
...
3. **Screenshots at the failing width** with `onespace-fe-verify-ui` — for
...
once (`onespace-fe-verify-ui`, "signed-in pages"), then pass
...
parallel (`onespace-fe-workflow/references/SUBAGENTS.md`); brief each with the
...
2. Apply the smallest fix that removes the cause, following
   `onespace-fe-coding-standards`. These are symptom masks, not fixes, and are
...
4. **Visual proof**: run `onespace-fe-verify-ui` on the affected pages at every
...
`onespace-fe-workflow` template, never to the repo root, the temp directory,
