---
name: onespace-fe-design-ui
description: >
  Premium, distinctive, responsive UI for the OneSpace React frontend — never a
  ...
  UI" — and when reviewing how a screen looks. Runs inside onespace-fe-workflow
  (types `ui`, `feature`, `verify`).
---

# Design UI

OneSpace's users judge the product in the first second, and a screen that looks
...
1. **Product and user** — "Team leads reviewing 200 requests a day on a
   laptop between meetings."
...
| Dial | Low (1–3) | High (8–10) | OneSpace app default | OneSpace marketing default |
|------|-----------|-------------|---------------------|---------------------------|
...
Every later choice follows the read and the dials. An operations dashboard with
...
of screen, screenshot them with `onespace-fe-verify-ui` (`--url`), and write
...
1. **Tokens** — add or confirm the tokens the direction needs
   ...
   (`onespace-fe-coding-standards` rule `style-tokens-only`).
...
After each `onespace-fe-verify-ui` pass, look at the screenshots as a demanding
...
- Every target page passes `onespace-fe-verify-ui` at every agreed width and
  theme.
