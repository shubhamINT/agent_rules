---
name: onespace-fe-coding-standards
description: >
  ALWAYS-ON OneSpace frontend coding standard for React + TypeScript — applies to
  ...
  yardstick for every other onespace-fe-* skill.
---

# Frontend Coding Standards

The best React code in a OneSpace repo is the code that was not written because
...
Always use what the repo already has; `onespace-fe-design-ui` says how to
pitch an upgrade.
...
1. Read the repo's `## Project structure` in `AGENTS.md`
   (`onespace-fe-structure-app` if empty). New files go where it says.
...
- Does any new file lack the BUSL-1.1 licence header (`onespace-fe-busl-licence-compliance`)?
...
- Did I add or change a script, env variable, dependency, or folder without
  updating the README (`onespace-fe-workflow/references/README-STANDARD.md`)?
