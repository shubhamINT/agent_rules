...
Usage:
    ...
    python add_license_headers.py --exclude 'src/generated/*' --exclude 'public/*'
...
# Generated by frameworks without our marker; never header them.
DEFAULT_EXCLUDES = ["next-env.d.ts", "*.gen.ts", "mockServiceWorker.js"]
...
def _html_header(lines: list[str]) -> str:
    """Render a header as an HTML `<!-- ... -->` comment."""
    body = "".join(f"  {line}\n" for line in lines)
    return f"<!--\n{body}-->\n"
...
HEADERS: dict[str, str] = {
    ...
    ".mts": _block_header(_FULL),
    ".cts": _block_header(_FULL),
    ".css": _block_header(_FULL),
    ".scss": _block_header(_FULL),
    ".html": _html_header(_SHORT),
...
def insert_header(text: str, header: str) -> str:
    ...
    if lines and lines[0].startswith("#!"):
        ...
        at = 1
    # CSS `@charset` and the HTML doctype must stay first.
    elif lines and (lines[0].startswith("@charset") or lines[0].lower().startswith("<!doctype")):
        at = 1
...
def iter_candidates(root: Path, excludes: list[str]):
    ...
    for path in sorted(root.rglob("*")):
        ...
        if any(part in SKIP_DIRS or part.endswith(".egg-info") for part in rel.parts):
            continue
        posix = rel.as_posix()
        if any(fnmatch.fnmatch(posix, pat) or fnmatch.fnmatch(path.name, pat) for pat in excludes + DEFAULT_EXCLUDES):
            continue
...
def _self_check() -> None:
    ...
    # Frontend: `@charset` and doctype stay first; "use client" may follow a comment.
    out = insert_header('@charset "utf-8";\n@import "tailwindcss";\n', HEADERS[".css"])
    assert out.startswith("@charset") and MARKER in out.splitlines()[3], out
    out = insert_header("<!doctype html>\n<html></html>\n", HEADERS[".html"])
    assert out.startswith("<!doctype html>\n<!--") and is_headered(out), out
    out = insert_header('"use client";\nexport {};\n', HEADERS[".tsx"])
    assert out.startswith("/**") and '"use client";' in out, out
    assert header_for(Path("app.scss")) and header_for(Path("index.html"))
